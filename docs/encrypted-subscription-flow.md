...
**Авторизация:** HTTP Basic Auth (admin credentials) — временное решение для bootstrap. В будущем будет заменено на сервисную авторизацию с access token.
...
Возвращается `HTTP 403 Forbidden` с телом, содержащим "отравленный" конфиг с одним фиктивным сервером. Это гарантирует, что HAPP-приложение перезапишет локальный кеш и не сможет продолжать использование VPN:
```
#profile-title: OvalVPN
#profile-update-interval: 1
...
Все HAPP-заголовки (поддержка, уведомления) сохраняются из оригинальной подписки. Мобильное приложение перезаписывает локальный кеш, и рабочие VPN-серверы исчезают.
...
- Expired/revoked подписки возвращают `HTTP 403 Forbidden` с «отравленным» конфигом (один фиктивный сервер)
